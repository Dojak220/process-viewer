...
void MainWindow::fillTable() {
    ...
    if (filter_text == "") {
        system("ps -eo pid,user,pri,nice,vsz,rss,trs,state,pcpu,pmem,time,command --sort=-pcpu > ps--eo.log");
    } else {
        sprintf(bash_command, "ps -eo pid,user,pri,nice,vsz,rss,trs,state,pcpu,pmem,time,command --sort=-pcpu | grep \"%s\" > ps--eo.log", filter_text.toLocal8Bit().constData());
        system(bash_command);
    }
...
{
//    Ainda nada
//    sprinf(comando, "renice %d -p %d", nc, pid);
//    system(command);
}
...
//void MainWindow::mudar_processo_de_cpu()
/*
{
//    Ainda nada
//    sprinf(comando, "taskset -cp %d %d", cpu, pid);
//    system(command);
}
*/
{
